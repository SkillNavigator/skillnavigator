...
useEffect(() => {
  const fetchSchedule = async () => {
    try {
      const userSettingId = 1;
      const response = await fetch(`http://localhost:8000/get-schedule/${userSettingId}`);
      const scheduleData = await response.json();
      if (Array.isArray(scheduleData)) {
        setSchedules(scheduleData);
      } else {
        console.error('Received data is not in the expected format:', scheduleData);
      }
    } catch (error) {
